...
**テーマ：** 97文字の暗号文と24文字のcribで、どの仕組みを否定できるか。見つけるのに使った自由度を数えた後で、生き残るパターンはどれか。

**現在地：** 復号はしていません。新しい平文文字は0です（2026-09-26時点）。K1〜K3と同じ「1枚の表のずらし」の形では、英語を隠す段を前後に置いても、彫り間違いを許しても、判定できる族はすべて候補なしか、cribでは判定できないかのどちらかでした。K4の文字頻度は平らなので鍵は1位置あたり3 bit以上が要り、解ける形は「短い技法」と「短く書ける鍵の出どころ」の組に限られます。その形を外した方式のうち、K4だけで判定できるもの（彫刻の文字列を表にする族）は反証され、判定できる別方式は今はありません。最新のResearch NoteはEP-0057で、「5つのWのうち2つが公開cribを外側から挟む」という観察は、Wという文字を選ばなければ確率0.036で、TOKIO読みとも独立ではありません。

**証拠の境界：** 公開暗号文と24文字のcribだけを使った探索的な計算です。事前登録はなく、外部の独立replicationも暗号専門家のレビューもありません。正解は存在しますが公開されていません。

**[現在のResearch Note（EP-0057）→](/ja/research/kryptos-k4-w-brackets/)** — Finding

[研究の現在地と更新履歴を見る →](/ja/programs/kryptos-k4/)
