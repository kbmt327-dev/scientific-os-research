"""E14: does alpha give a boundary that stops moving?

Predictions and grading rules are sealed in predictions/PRED-013.json before
this script is run.

EP-0011 showed flow balance is a censoring ratio and cannot see divergence.
The candidate replacement is alpha, the elasticity of the largest class's mean
response time with respect to the observation window.  This run adds a fourth
horizon (240000 jobs) and three more seeds so that the boundary alpha reports
can be tested for a fixed point.
"""
import json
import os
import sys
...
P_LEVELS = [0.002, 0.02]
HORIZONS = [30_000, 60_000, 120_000, 240_000]
SEEDS = [901, 902, 903, 904, 905, 906, 907, 908]
EASY_RATIOS = [0.875, 1.0]
EASY_P = 0.002
EASY_SEEDS = [901, 902, 903, 904]
...
KEEP = ["mean_jct", "utilization", "rho_emp", "mean_backlog", "mean_running",
        ...
        "completion_frac_by_need", "starving_needs"]
...
def one(task):
    ...
    cfn = sim["completion_frac_by_need"]
        ...
        for ratio in RATIOS:
            for p in P_LEVELS:
                for n_jobs in HORIZONS:
                    for seed in SEEDS:
                        tasks.append(("A", e_label, ratio, p, n_jobs, seed,
                                      "srpt"))
    for e_label in E_TARGETS:
        for ratio in EASY_RATIOS:
            for n_jobs in HORIZONS:
                for seed in EASY_SEEDS:
                    tasks.append(("E", e_label, ratio, EASY_P, n_jobs, seed,
                                  "easy_backfill"))
    # longest and largest-ratio first: those dominate the tail
    tasks.sort(key=lambda t: (-t[4], -t[2]))
    return tasks
...
if __name__ == "__main__":
    ...
    print(f"E14: {len(tasks)} runs {by_arm}  workers={workers}  N={N}",
          flush=True)
    print(f"  horizons={HORIZONS}  seeds={SEEDS}", flush=True)

    rows, t0 = [], time.time()
    with ProcessPoolExecutor(max_workers=workers) as ex:
        for i, rec in enumerate(ex.map(one, tasks), 1):
            rows.append(rec)
            if i % 60 == 0 or i == len(tasks):
                print(f"  {i}/{len(tasks)}  ({time.time() - t0:.0f}s)",
                      flush=True)

    out = os.path.join(HERE, "results", "E14.json")
    with open(out, "w", encoding="utf-8") as handle:
        json.dump({"config": {
            ...
            "easy_seeds": EASY_SEEDS, "geom_support": list(GEOM_SUPPORT),
