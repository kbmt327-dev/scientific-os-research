...
class Job:
    ...
    n_preempt: int = 0
    running: bool = False
    # heterogeneous engine only; rate 1.0 and no assignment is the homogeneous
    # case and is never touched by the fungible-server path.
    rate: float = 1.0
    servers: tuple = ()
...
GEOM_NEEDS = (1, 2, 4, 8, 16, 32, 64)


def geometric_mix(theta: float, needs=GEOM_NEEDS):
    """One-parameter family of gang-need mixes: P(need = 2^i) proportional to
    theta^i. theta < 1 is dominated by single-GPU jobs, theta = 1 is uniform over
    the powers of two, theta > 1 is dominated by whole-cluster gang jobs.
    Gives a continuous axis between trace-like and gang-heavy workloads."""
    w = np.array([theta ** i for i in range(len(needs))], dtype=float)
    probs = tuple(w / w.sum())
    return tuple(needs), probs


def mix_stats(needs, probs, n_servers):
    """Descriptors of a mix: mean need, and the share of offered work carried by
    jobs needing at least a quarter of the cluster."""
    needs = np.asarray(needs, dtype=float)
    probs = np.asarray(probs, dtype=float)
    e_need = float(np.dot(needs, probs))
    big = needs >= n_servers / 4
    return {"e_need": e_need, "e_need_over_N": e_need / n_servers,
            "work_share_big": float(np.dot(needs[big], probs[big]) / e_need),
            "p_full": float(probs[needs >= n_servers].sum())}
...
def make_workload(
    ...
    est_mode:
      "mean"   est = size * bias * exp(sigma*Z - sigma^2/2). E[est|size] = size,
               but the MEDIAN estimate shrinks by exp(-sigma^2/2), so a large
               sigma also means the typical job is systematically underestimated.
      "median" est = size * bias * exp(sigma*Z). The median estimate equals the
               size at every sigma, so sigma is pure spread. Use this to
               separate noise from systematic underestimation (EP-0001 U-2).
    """
    ...
    if sigma > 0.0:
        ...
        shift = 0.0 if est_mode == "median" else 0.5 * sigma * sigma
        est = size * est_bias * np.exp(sigma * z - shift)
    else:
        est = size * est_bias
